...
const sendmailInputs = document.querySelectorAll(".sendmail-item");
...
signupForm.addEventListener("submit", e => {
    ...
    .then(response => {
        ...
        if (response.status === 200) {
            sendmailInputs.forEach(item => item.classList.toggle("hidden"));
            signupInputs.forEach(item => item.classList.toggle("hidden"));
        } else {
            alert("회원가입 실패")
        }
    })
...
authCheckForm.addEventListener("submit", e => {
    e.preventDefault();
    const email = document.getElementById("signup-email");
    const data = new FormData(authCheckForm);
